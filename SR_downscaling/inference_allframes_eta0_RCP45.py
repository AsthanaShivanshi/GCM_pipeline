...
tas_dir = f"{config.BIAS_CORRECTED_DIR_RCP45}/CDFT"
pr_dir = f"{config.BIAS_CORRECTED_DIR_RCP45}/CDFT"
...
for tas_path in tas_files:
    ...
    for idx in tqdm(range(len(ds)), desc="Downscaling RCP45 frames for 4 runs"):
    ...
    out_path = f"EUR-CH(SR)/UNet_downscaled_RCP45_CDFT_{os.path.basename(tas_path)}"
    ds_out = xr.Dataset(
        {
