...
def pooled_parallel(files, varname='pr', mask=None):
    def process_file(file):
        ...
        masked = np.clip(masked, 0, None)  # Clip to non-negative
        ds.close()
        if masked.size > 0:
            return masked
        else:
...
data_nonnegative = [np.clip(d, 0, None) for d in data_nonempty]


data_eps = [np.where(d == 0, EPS, d) for d in data_nonnegative]
log_data_nonempty = [np.log10(d) for d in data_eps]

#--------------------------------

plt.figure(figsize=(14, 9))
log_range = (np.log10(EPS), np.log10(300))
