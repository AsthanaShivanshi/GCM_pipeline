...
def _to_valid_datetime64(ds, var_name):
    ...
    for idx, t in enumerate(da_raw["time"].values):
        try:
            time_values.append(
                np.datetime64(f"{t.year:04d}-{t.month:02d}-{t.day:02d}", "ns")
            )
...
def cdft_cell(
    ...
    if (
        ...
        return corrected, correction_obtaining_seconds, correction_applying_seconds
    ...
    common_calib_dates = np.intersect1d(
        calib_model_dates[calib_mask_m],
        obs_dates[calib_mask_o],
    )
...
def main():
    ...
    for gcm in ["EC-Earth3-Veg", "MPI-ESM1-2-HR", "NorESM2-MM"]:
        for var in ["tas", "pr"]:
            ...
            if hist_dsf is None:
                print(f"No valid historical times in {hist_path}")
                hist_ds.close()
                continue
            ...
            for period in ["historical", "ssp370"]:
                ...
                if model_path is None:
                    print(f"Missing target file for {gcm} | {var} | {period}")
                    continue
                ...
                if model_var not in model_ds.data_vars:
                    print(f"Variable '{model_var}' not found in {model_path}")
                    model_ds.close()
                    continue
                ...
                if model_dsf is None:
                    print(f"No valid model times in {model_path}")
                    model_ds.close()
                    continue
