...
def qdm_cell(model_cell, obs_cell, model_times, obs_times, var_name):
    ...
    # Clip precip for nonneg values : QUALIFIER
    if var_name == "precip":
    ...
    for doy in range(1, 367):
        ...
        qdm_mode= "multiplicative" if var_name == "precip" else "additive" #Wang et al.,2024
#Delta method: "additive" or "multiplicative". It is possible to pass custom delta function with a tuple where the first element is the transform ( e.g. np.add or np.multiply) and the second one its inverse (e.g. np.subtract or np.divide)
        ...
        qdm = QDM(bin_width=None, bin_origin=None, mode=qdm_mode)
        ...
        if var_name == "precip":
            corrected_full = np.clip(corrected_full, 0, None) #CLIPPING PRECIP : QUALIFIER, to be mentioned in the proposal
...
def main():
    ...
    for model_path in tqdm(tas_files, desc="Processing QDM tas"):
