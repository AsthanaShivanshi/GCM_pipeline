...
ALLOWED_IDS = {
    "EUR-11_DMI-HIRHAM5_ICHEC-EC-EARTH_r3i1p1_rcp26_1971-2099.nc",
    "EUR-11_MPI-CSC-REMO2009_MPI-M-MPI-ESM-LR_r1i1p1_rcp26_1971-2099.nc",
    "EUR-11_SMHI-RCA4_ICHEC-EC-EARTH_r12i1p1_rcp26_1971-2099.nc"
}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, choices=["bicubic", "unet", "ddim"], required=True)
    parser.add_argument("--ensemble", type=str, choices=["EQM", "dOTC", "CDFT"], required=True, help="BC name (EQM, dOTC, or CDFT)")
    args = parser.parse_args()

    tas_dir = f"{config.BIAS_CORRECTED_DIR_RCP26}/{args.ensemble}"
    pr_dir = f"{config.BIAS_CORRECTED_DIR_RCP26}/{args.ensemble}"
    bicubic_dir = f"{config.BIAS_CORRECTED_DIR_RCP26}/{args.ensemble}"
...
#----------------------------------------------------------------------#
        ...
        tas_files = [f for f in tas_files if get_id(f, "tas") in ALLOWED_IDS]
        pr_files = [f for f in pr_files if get_id(f, "pr") in ALLOWED_IDS]
        pr_dict = {get_id(f, "pr"): f for f in pr_files}
